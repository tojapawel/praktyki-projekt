                ...
                <div className="col-md-8">
                    <div className="card-body">
                        ...
                        <p className="card-text">
                            ...
                            <small>{hotel.distanceFromCenter} km {t("hotels.hotelCard.toCenter")}</small>
                        </p>
                        <small className="ml-2">{t("hotels.filter.metadata.wifi")} {setIcon(hotel.wifi)}</small>
                        <small className="ml-2">{t("hotels.filter.metadata.parking")} {setIcon(hotel.parking)}</small>
                        <small className="ml-2">{t("hotels.filter.metadata.pets")} {setIcon(hotel.pets)}</small>
                        <small className="ml-2">{t("hotels.filter.metadata.roomService")} {setIcon(hotel.roomService)}</small>    
                    </div>
                </div>
                <div className="col-md-4">
                    <div className="card-body text-end">
                        <h6>{t("hotels.hotelCard.reviewScore")} <span className="badge bg-primary">{hotel.reviewScore}</span></h6>
                        <h6><CalculateStars stars={hotel.stars}/></h6>
                        <h6>{getMinPrice(props.rooms)} zł / {t("hotels.hotelCard.night")} <MdHelpOutline style={{position: 'relative', bottom: '1px'}} className='price'/></h6>
                        <Link to={`/hotel/${hotel.hotel_id}`} type="button" className="btn btn-primary btn-sm">{t("hotels.hotelCard.showPrices")}</Link>
                    </div>
                </div>
            </div>
        </div>
