<?php
    ...
    foreach ($data as $hotel) {
        ...
        var_dump((float)$hotel['reviewsScore']);
        $reviewsScore = (float)$hotel['reviewsScore'];

        $sql = "INSERT INTO hotels (hotel_id, name, city, code, address, stars, distanceFromCenter, wifi, parking, pets, roomService, reviewScore, promoted) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"; //TODO: zmienić '?' na odpowiednie wartości

        $stmt = $conn->prepare($sql);
        $stmt->bind_param("sssssiiiiiiid", $hotel['id'], $hotel['name'], $hotel['location']['city'], $hotel['location']['postCode'], $hotel['location']['address'], $hotel['stars'], $hotel['metadata']['distanceFromCenter'], $hotel['metadata']['wifi'], $hotel['metadata']['parking'], $hotel['metadata']['pets'], $hotel['metadata']['roomService'], "$reviewsScore", $hotel['promoted']);
        $stmt->execute();
